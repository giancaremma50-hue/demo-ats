...
12. **Todo campo dice qué le pasa, y todo campo enfocado se ve.**
   ...
   **Excepción declarada**: cuando el campo comparte fila con su botón (`InviteForm`, `AddRejectionReasonForm`), el mensaje va **debajo de la FILA**. Adentro de la columna del campo, con la fila en `items-end`, fallar empuja el botón hacia abajo y desalinea la fila entera justo en el momento en que el usuario está mirando.
   ...
   **Migración en curso — la regla vale para código nuevo desde ya, y para el viejo a medida que se toca.** Al 2026-09-13 pasan por `<Field>`/`<FieldError>` 9 de los 25 formularios: el público de postulación, vacantes, marca, invitación, motivos de rechazo, mensaje al candidato, seguimientos, descarte y referir. Los otros 16 (los seis pasos del asistente de plantillas de puesto, nueva vacante, departamentos, plantillas de mensaje, tareas, agendar reunión, colaboradores, segmentos, bolsa, reportar error e hilo de error) **siguen avisando solo con toast** y son deuda declarada, no precedente: ningún formulario nuevo se escribe así, y un formulario migrado que vuelva a quedarse en toast es un hallazgo de review.

   **Qué cuenta como "tocarlo".** Tocar su manejo de errores, sus campos o su envío obliga a migrarlo en la misma entrega. Un barrido mecánico de todo el repo —quitar una clase, renombrar un token— **no**: exigirlo convertiría cualquier limpieza de una línea en catorce migraciones, que es cómo se deja de hacer limpiezas. Esa es exactamente la excepción con la que entró esta entrega: barrió `outline-none` de 20 archivos —los 21 que tenían la clase menos `ui/popover.tsx`, que es la excepción declarada— y migró solo los formularios cuyos errores sí tocó.
