...
10. **La barra flotante: tres anclas fijas, y en medio lo que cambia.**
   `[⊞ Módulo] · [Inicio] [pantallas del módulo] · [⚙]` (mockup aprobado el 2026-09-11 y reaprobado el 2026-09-13 con el mapa en el selector). El **selector de módulos**, el **Home** y el **engranaje** están en toda pantalla y en la misma posición; nunca desaparecen — una pantalla sin la puerta de vuelta a la principal es un callejón sin salida, y eso pasó al entrar a Configuración.
...
11. **Cada módulo trae su propio submenú, y fuera de un módulo no se muestra ninguno.**
   Regla de diseño para todo módulo nuevo (aprobada el 2026-09-13, tras el reporte: *"cada menú debe tener su agrupación; ahorita que está en Inicio no debería de verse lo de Vacantes, Candidatos y Bolsa, eso son submenú del ATS"*).

   | Dónde estás | Qué muestra la barra |
   |---|---|
   | En un módulo (`/vacantes`, `/candidatos`, `/bolsa`, `/conectados`) | `[⊞ Módulo] · [Inicio] [las pantallas de ESE módulo] · [⚙]` |
   | Fuera de todo módulo (`/inicio`, `/configuracion`, `/mi-cuenta`, `/notificaciones`) | `[⊞] · [Inicio] · [⚙]` — las tres anclas y nada más |

   - **Un módulo nuevo declara sus pantallas en `itemsForRole`, por rol — y con eso ya declaró sus rutas.** La pertenencia se deriva de los `href` de esas pantallas, en todos los roles: agregar una pantalla trae su ruta consigo, sin una segunda lista que se pueda desincronizar. Lo único aparte es `extraRoutes`, opcional, para las rutas que **no** son destino del menú (`/postulaciones/<id>` es del ATS y no tiene ítem propio).
   - **Lo que nadie declara no es de nadie.** `activeModuleFor` devuelve `null` y la barra queda en sus tres anclas. No hay fallback a Reclutamiento ni lista aparte de rutas "sin módulo": esa lista existió y era una trampa —toda pantalla nueva nacía heredando los submenús del ATS hasta que alguien se acordara de anotarla—, que es el bug de este reporte esperando para repetirse.
   - **Las pantallas del medio pertenecen al módulo activo y a nadie más.** Mostrar las de otro es lo que rompió esta barra: `/inicio` era una pantalla de Reclutamiento, así que parado en la portada se veían Vacantes, Candidatos y Bolsa, que son del ATS.
   - **Inicio no es de ningún módulo**: es la portada de la plataforma, igual que Ajustes. Ningún módulo declara su ruta, así que `activeModuleFor("/inicio")` devuelve `null`; su ítem lo pone la barra (`HOME_ITEM`), no `itemsForRole`.
   - **El selector `⊞` es el mapa completo**: cada módulo y, colgando, todas sus pantallas para ese rol. Es lo que permite vaciar el medio de la barra sin dejar nada inalcanzable — desde Inicio se llega a Candidatos en dos toques, sin pasar por Vacantes. Un módulo que no aparezca ahí queda escondido.
   - **El nombre del módulo en el mapa es un encabezado, no un enlace.** Su destino sería el de su primera pantalla, que está justo debajo: dos controles contiguos al mismo lugar es la ambigüedad que ya costó un arreglo en esta barra.
   - **Costo aceptado — el eco del mapa**: con el mapa abierto, "Vacantes" existe dos veces en el árbol de accesibilidad (el ítem de la barra y su fila en el mapa). No es el problema que arregló la regla 10 —ahí eran dos controles con el mismo nombre y **destinos distintos**, y uno nombraba al otro—: acá los dos llevan al mismo lado, así que desambiguar por voz da lo mismo elija lo que elija. Se acepta a cambio de que el mapa esté completo; un mapa que esconde la pantalla donde estás deja de ser un mapa.
   - **Dos pantallas no pueden llamarse igual en el mapa.** Por eso el muro de AJE Conectados se llama **Muro** y no "Inicio": con los dos módulos diciendo "Inicio" el mapa mostraba dos filas idénticas, y en un teléfono la barra decía lo mismo en los dos módulos. Esto reemplaza al "Límite conocido" de la regla 10, que describía justo ese problema.
   - **El nombre de la PANTALLA se muestra en teléfono aunque la ruta no tenga módulo.** Lo que se calla fuera de un módulo es el nombre del MÓDULO, que sería falso; la pantalla existe igual, y en Inicio o en Ajustes ese texto es lo único que la nombra cuando la etiqueta del ítem activo va oculta por espacio. **Límite**: solo lo tienen las rutas que son destino de algún ítem de la barra — `/mi-cuenta` y `/notificaciones` no lo son, así que ahí el teléfono sigue sin nombrar la pantalla. Se cierra el día que esas dos tengan ancla propia.
   - **El `h1` de una pantalla empieza por el nombre con que la nombra el menú.** Puede ser más largo ("Bolsa" en la barra, "Bolsa de empleo" en la pantalla), nunca otra cosa: un `h1` que dice algo distinto del ítem que llevó hasta él no confirma que hayas llegado a donde tocaste. Por eso el muro pasó de "AJE Conectados" —que nombra al MÓDULO, no a la pantalla— a "Muro". **Excepción declarada**: en `/inicio` el `h1` es el saludo ("Buenos días, Giancarlo"), que nombra a la persona; ahí quien nombra la pantalla es la barra.
   - **Un paso del tour solo puede apuntar a algo presente en TODA ruta autenticada.** `OnboardingTour` se monta en `(app)/layout.tsx`, así que corre en la primera pantalla que cargue —puede ser un enlace a `/postulaciones/<id>` desde una notificación—, filtra sus pasos por si el elemento está en ese DOM y después marca el tutorial como visto para siempre. Un paso que apunte a un ítem de módulo se descarta en silencio y nadie lo ve nunca. Van a las tres anclas, la campana y Mi cuenta; nunca a un submenú.
   - **Costo aceptado — el verde**: fuera de todo módulo la barra va en `--primary`, que es el color de la plataforma (logo, botón principal) y también el de Reclutamiento. En un teléfono, donde el módulo se distingue por el color, venir de Conectados a Inicio se ve como entrar a Reclutamiento; lo que lo despeja es el nombre de la pantalla, que ahí sí se pinta. Se resuelve el día que Reclutamiento tenga un color propio distinto del de la marca.

12. **Todo campo dice qué le pasa, y todo campo enfocado se ve.**
   Regla nacida de la auditoría del 2026-09-13, que encontró las dos cosas rotas a la vez y por la misma causa: el producto ya tenía la pieza correcta y algo la apagaba.

   **El error va DEBAJO del campo, con color Y texto.** Nunca solo color —un borde rojo no lo ve quien no distingue el rojo— y nunca solo un toast: el toast se va solo, no dice qué campo, y no deja nada en pantalla. El toast queda para el error que **no es de ningún campo** ("No se pudo guardar"), que es donde sí corresponde.
   **Excepción declarada, por condición y no por lista**: cuando el campo comparte fila con su botón, el mensaje va **debajo de la FILA**. Adentro de la columna del campo, con la fila en `items-end`, fallar empuja el botón hacia abajo y desalinea la fila entera justo en el momento en que el usuario está mirando. Esas filas no pasan por `<Field>` —que apila en columna— sino por `camposDeFila` (`src/lib/forms/use-error-toast.ts`), que da las mismas piezas desde una sola fuente; un control suelto con layout propio usa `campoSuelto`.

   **Un grupo de controles no es un campo.** Un conjunto de casillas (`CollaboratorsPicker`) o un control con su propia fila de alta (`EmploymentReasonSelect`) no puede ser el hijo único de `<Field>`: se nombra con `role="group"` + `aria-labelledby`, o con un `<label htmlFor>` que apunte al control real, y recibe el mensaje por props (`error` + `idError`) para pintarlo con `<FieldError>`. El id del mensaje lo arma el formulario padre, que es el mismo que `useErrorToast` va a buscar.

   **Un campo con paso del tour lleva su `data-tour` en el `<Field>`**, que lo pone en el contenedor. Sin eso, migrar un campo del asistente le borraba el anclaje de su paso — y un paso sin elemento se descarta en silencio y no lo ve nadie nunca (regla 11).

   **Los ids de un formulario salen de `useId()`, uno por FORMULARIO.** La pantalla de plantillas de mensaje monta un diálogo por plantilla más el de "nueva": con ids fijos, `htmlFor` y `aria-describedby` resuelven por la PRIMERA coincidencia del documento y todos apuntan al primero. Y es por formulario, no por componente: `collaborators-panel` tiene dos y necesita dos `useId()`, o el día que sus schemas compartan un nombre de campo cada uno se callaría el toast por el mensaje del otro.
   **Excepción viva, y es deuda, no patrón**: seis formularios de la primera entrega (`marca-`, `vacante-`, `referir-`, `rechazo-`, `mensaje-` y el público de postulación con `postular-`) todavía llevan prefijo escrito a mano. Hoy no chocan porque cada uno se monta una sola vez por pantalla — pero eso nadie lo garantiza hacia adelante, y el que los toque los pasa a `useId()`.

   **Todo campo va dentro de `<Field>`** (`src/components/ui/field.tsx`), que hace por su cuenta lo que no puede depender de que alguien se acuerde: etiqueta visible, mensaje debajo, `aria-invalid` en el control, `aria-describedby` apuntando al mensaje, y `role="alert"` para que se anuncie al aparecer. Antes de la auditoría había **cero** `role="alert"` en todo el proyecto y **un solo** `aria-describedby` real (`media-picker.tsx`; el segundo resultado del grep era un comentario que decía justamente "Sin aria-describedby"): los pocos errores que se pintaban se veían, pero no se anunciaban ni estaban atados a su campo. Un control con layout propio que no pueda pasar por `<Field>` usa `<FieldError>` y **cablea a mano las tres cosas**.

   **Una etiqueta visible, nunca un placeholder como etiqueta.** El placeholder desaparece al escribir: el usuario deja de saber qué está llenando, y si el formulario le devuelve un error no tiene con qué ubicarlo. El formulario público de postulación era así, y es el único que usa alguien de afuera, sin cuenta ni soporte.

   **Toda validación de Zod en una Server Action pasa por `zodFieldError`** (`src/lib/forms/zod-error.ts`). Sin `field`, el mensaje no tiene dónde ir y termina de vuelta en un toast genérico. Un `return { error: parsed.error.issues[0]?.message }` es un hallazgo de review.

   **Ningún control lleva `outline-none`.** La regla `:focus-visible` de `globals.css` —2px de anillo en `--ring`, que pasa 5.01:1— es la única fuente del foco, y ya está bien hecha. Y ojo con cómo se rompe: `outline-none` **no gana por cascada ni por especificidad**, deja `--tw-outline-style` en `none` de forma permanente, así que el anillo **no se dibuja nunca**, en ningún navegador. Es lo que hacía que hasta el único archivo que intentaba reponerlo (`focus-visible:outline-ring`) tampoco funcionara: esa utilidad lee la misma variable. **Única excepción declarada**: el contenedor del popover (`ui/popover.tsx`), que Radix enfoca por código para meterte adentro — un anillo alrededor de todo el panel es ruido, y los controles de adentro tienen el suyo.

   **El foco no se señala con el borde.** El borde es del error y de la estructura; el anillo es del foco, y va por fuera (`outline-offset: 2px`) para que los dos convivan.
   **Excepción declarada — un anillo `focus:` propio donde el foco lo da el CÓDIGO**: los dos contenedores `role="group"` que `useErrorToast` enfoca al llevar al usuario hasta el campo (`CollaboratorsPicker` y el grupo de destinatarios de `MeetingScheduler`). `:focus-visible` no matchea un foco programático salvo que la última interacción haya sido de teclado, así que sin un `focus:` propio el foco caía en un destino invisible — y la regla 6 pide foco visible siempre.

   **Excepción declarada — el anillo hacia adentro** (`outline-offset` negativo) en un control que vive dentro de un contenedor que recorta: la tira de pestañas de Ajustes (`config-tabs.tsx`, su `overflow-x-auto` recorta también el eje Y) y el área de respuesta del hilo de errores (`error-thread.tsx`, dentro de una `<Card>` con `overflow-hidden`). Ahí un anillo por fuera se corta y se ve como dos barras sueltas; darle aire al contenedor lo arregla y rompe otra cosa (en las pestañas despegaba el subrayado de la activa del riel de abajo). Cuando los dos usaban el borde, la regla con pseudo-clase ganaba: al entrar a un campo inválido para corregirlo **el rojo se volvía verde** y la señal de error desaparecía justo cuando hacía falta. Dos señales distintas, dos canales distintos.

   **Y el error se marca por más de un canal**: borde de 2px y fondo tenue, no solo el color del borde. El grosor se nota sin depender de ver el rojo. **La cadena vive en un solo lugar** —`ERROR_CONTROL_CLASS` en `ui/field.tsx`, y `ERROR_CONTROL_TINT` para la excepción de 1px— y los controles cableados a mano la importan; escrita a mano en cada archivo, cambiarla significaba editar seis lugares y no notar el séptimo. El fondo tenue es el token literal `--destructive-soft`, **nunca `bg-destructive/5`**: esa utilidad compila a `color-mix()` con el color ENTERO de respaldo, así que en un navegador viejo el campo en error se pintaba de rojo sólido con el texto encima a 3.5:1.
   **Excepción declarada**: la caja de seguimientos (`note-form`) se queda en 1px. Es un `<textarea>` transparente encima de un div que pinta el mismo texto, y los dos tienen que medir EXACTAMENTE lo mismo o el cursor se desalinea de las letras — un borde de 2px cambia la caja. Ahí el segundo canal lo dan el fondo tenue y el mensaje.

   **El toast lo decide `useErrorToast`, no una lista escrita a mano** (`src/lib/forms/use-error-toast.ts`). Pregunta si el mensaje **está pintado** —`document.getElementById(id)` y `getClientRects().length`— y solo entonces se calla. Que el nodo esté montado no alcanza: dentro de un `<dialog>` que el usuario cerró mientras la acción corría, el mensaje existe y no se ve. La primera versión de esto fue un `Record<string, boolean>` copiado en siete formularios, y falló como falla toda lista paralela (regla 11): decía "se ve" de un campo que no se veía, y el error quedaba mudo. **Y no quedó ninguna lista**: el formulario público de postulación pinta su propio mensaje general junto al botón, y lo hace con la única condición que sale de su estado —el error **no trae campo**—, nunca de saber qué campos están en pantalla. Al hook se le pasa el id de ese mensaje general (`useErrorToast(state, mapa, "postular-error")`) y él decide igual que siempre: si no se ve ni el del campo ni el general, habla el toast.

   **Y pintado no es lo mismo que visto: cuando el mensaje sí está en pantalla, se lleva al usuario hasta él.** Foco al control que lo tiene por `aria-describedby` y `scrollIntoView`. En un formulario largo el mensaje puede quedar a dos pantallas del botón que se acaba de tocar, y sin toast y sin nada que se mueva lo único que el usuario ve es que el botón dejó de girar. **Y si no se puede llevarlo —el foco está en otro formulario de la misma pantalla, y moverle la vista sería peor— el toast vuelve a hablar.** Siempre queda exactamente una señal: o el salto al campo, o el toast; nunca ninguna, nunca las dos. **Límites conocidos**, los dos sin resolver: `visibility:hidden` y `opacity:0` cuentan como pintados (nadie esconde así un mensaje hoy, pero un cajón que se anime saliendo lo haría), y si el componente se desmonta con la mutación en vuelo —cambiar de pestaña en el cajón del candidato— no queda nadie para avisar, ni por toast ni debajo del campo.

   **Migración terminada (2026-09-13).** **27 formularios** —26 instancias de `useActionState` en 24 archivos, más el público de postulación, que usa `fetch`— y **los 27 pasan por `useErrorToast`**. Se cuentan instancias, no archivos: `wizard-step6-form` y `collaborators-panel` tienen dos cada uno.

   **23 de los 27 muestran el mensaje debajo del campo.** Los otros 4 —los pasos 3 y 5 del asistente y las dos acciones del paso 6— no tienen **ningún control que el error pueda nombrar**: el de `questions` habla de un editor de filas, no de un campo; el paso 5 es un switch que no puede fallar; el 6 no valida nada. Ahí el toast es el canal correcto, no una deuda.

   **Cuidado con "el schema es anidado, así que no hay campo":** es cierto para `stages`/`questions` **dentro** del arreglo, pero el error del arreglo en sí (`["questions"]`) tiene UN segmento y sí devuelve `field`. Lo que decide no es la forma del schema, es si existe un control con ese id. Esa frase estuvo escrita acá y era falsa para dos de los cuatro pasos.

   **Lo que queda fuera, y por qué**: los compositores de AJE Conectados (`post-composer`, `comment-thread`) no usan `useActionState` —llaman a la acción dentro de un `useTransition`— y sus avisos son validaciones previas del cliente, sin campo. Migrarlos es trabajo aparte, no deuda de esta regla.

   **Un formulario nuevo nace migrado**, y uno que vuelva a quedarse en toast teniendo un campo que señalar es un hallazgo de review.

   **Qué cuenta como "tocarlo".** Tocar su manejo de errores, sus campos o su envío obliga a dejarlo conforme en la misma entrega. Un barrido mecánico de todo el repo —quitar una clase, renombrar un token— **no**: exigirlo convertiría cualquier limpieza de una línea en catorce migraciones, que es cómo se deja de hacer limpiezas. Esa fue la excepción con la que entró la primera entrega, que barrió `outline-none` de 20 archivos —los 21 que tenían la clase menos `ui/popover.tsx`, la excepción declarada— y migró solo los formularios cuyos errores sí tocó.

   **El toast se calla según si el mensaje SE MUESTRA, no según si el error trae campo.** Es la trampa que esta misma entrega introdujo dos veces y el review encontró las dos: un campo puede estar escondido por configuración (`candidacyFields`, `departments.length > 0`), así que un error con `field` puede no tener dónde mostrarse. Silenciar el toast por tener `field` deja el formulario **fallando en silencio**, que es el peor final posible (regla 5). La condición se deriva de lo mismo que decide si el campo se pinta, nunca de una lista aparte.
