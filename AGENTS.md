...
10. **La barra flotante: tres anclas fijas, y en medio lo que cambia.**
   `[⊞ Nombre del módulo] · [Home] [submenús del módulo] · [⚙]` (mockup aprobado el 2026-09-11, variante B). El **selector de módulos**, el **Home** y el **engranaje** están en toda pantalla y en la misma posición; nunca desaparecen — una pantalla sin la puerta de vuelta a la principal es un callejón sin salida, y eso pasó al entrar a Configuración.
   **El nombre del módulo vive dentro del selector**, no en una ficha aparte: ese botón ya es el que dice en cuál estás y el que deja cambiar, y así ahorra el lugar que en un teléfono se agota primero. Por lo mismo, **no se muestra donde sería falso**: en `/configuracion`, `/mi-cuenta` y `/notificaciones` el selector va sin nombre (ver `isModulelessPath`).
