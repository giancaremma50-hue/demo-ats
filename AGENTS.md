...
10. **La barra flotante: tres anclas fijas, y en medio lo que cambia.**
   `[⊞ Módulo] · [Home] [submenús del módulo] · [⚙]`, con la etiqueta del ítem activo a la par de su ícono (mockup aprobado el 2026-09-11, variante B; la etiqueta se corrigió el 2026-09-13 contra la app real — el mockup había elegido el nombre del módulo). El **selector de módulos**, el **Home** y el **engranaje** están en toda pantalla y en la misma posición; nunca desaparecen — una pantalla sin la puerta de vuelta a la principal es un callejón sin salida, y eso pasó al entrar a Configuración.
   **En un teléfono el texto es para la PANTALLA, no para el módulo** (corregido 2026-09-13, con la app a la vista: con Home seleccionado la barra decía "Reclutamiento" y nada decía "Inicio"). El ítem activo muestra su etiqueta en **todo ancho**; el nombre del módulo cede el lugar y aparece recién en `sm:`. El módulo no queda mudo: lo dicen el color de la barra y el popover, que lo nombra completo al abrirlo.
   ...
   **El texto visible de un control nombra a ESE control.** No se le pone a un botón el nombre de la pantalla a la que lleva otro: el selector abre el selector de módulos, así que dice el módulo, y su `aria-label` coincide con lo que se ve (WCAG 2.5.3). Poner "Inicio" dentro del botón que abre el selector fue el primer intento de este arreglo, y estaba mal.
